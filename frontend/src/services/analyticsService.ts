...
class AnalyticsService {
  ...
  /**
   * Get study history (sessions grouped by date)
   */
  async getStudyHistory(userId: string, days: number = 30): Promise<{
    date: string;
    ...
    try {
      const sessions = await focusService.getUserSessions(userId, 1000);
      ...
      const history = Object.entries(grouped)
        .map(([date, sessions]) => ({
          ...
          totalFP: sessions.reduce((sum, s) => sum + s.focusPointsEarned, 0),
   ...
   */
  async getSubjectBreakdown(userId: string): Promise<{
    subjectName: string;
    icon: string;
    color: string;
    totalMinutes: number;
    ...
    try {
      const subjectData = await focusService.getSubjectStudyData(userId);
      ...
      return subjectData
        .map((subject) => ({
          ...
          percentage: Math.round((subject.totalMinutes / totalMinutes) * 100),
        }))
