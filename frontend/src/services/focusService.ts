...
class FocusService {
  ...
  async startFocusSession(
    ...
    try {
      const response = await apiCall('/focus/sessions/start', 'POST', {
        duration,
        cycles,
  ...
  async completeFocusSession(
    ...
    try {
      const response = await apiCall(`/focus/sessions/${sessionId}/complete`, 'POST', {
        actualDuration,
        cyclesCompleted,
        distractionCount,
        pauseCount,
   ...
   */
  async getUserSessions(userId: string, limitCount: number = 30): Promise<FocusSession[]> {
    try {
  ...
  async getSubjectStudyData(userId: string): Promise<SubjectStudyData[]> {
    try {
      const response = await apiCall('/focus/subjects', 'GET');
  ...
  async getFocusStats(userId: string): Promise<{
    ...
    try {
      const response = await apiCall('/focus/stats', 'GET');
  ...
  private async startBlockingForSession(userId: string, duration: number): Promise<void> {
    try {
      const blockedApps = await blockingService.getUserBlockedApps(userId);
      const activeApps = blockedApps.filter((app) => app.blocked);

      for (const app of activeApps) {
        await blockingService.startBlockingSession(userId, app.id, app.name, duration);
      }
    } catch (error) {
  ...
  private async completeBlockingForSession(userId: string): Promise<void> {
    try {
      const activeSessions = await blockingService.getActiveBlockingSessions(userId);

      for (const session of activeSessions) {
        await blockingService.completeBlockingSession(session.id, userId);
      }
    } catch (error) {
  ...
  private async breakBlockingForSession(userId: string): Promise<void> {
    try {
      ...
      for (const session of activeSessions) {
        await blockingService.breakBlockingSession(session.id, userId);
      }
    } catch (error) {
